...
public class Pagination {
    ...
    public Pagination(boolean autoComput) {
        this(DEFAULT_PAGE_SIZE, DEFAULT_CURRENT_PAGE, DEFAULT_SKIP_SIZE);
     ...
     */
    public int getNext() {
        if (this.currentPage < this.pageCount) return this.currentPage + 1;
        else return this.pageCount;
    }
    ...
    public void setUrl(HttpServletRequest request){
        String headUrl = request.getRequestURI();
        ...
        if (queryString != null) {
            ...
            for (int i = 0; i < parameterPairs.length; i++) {
                ...
                if (kvs[0].equals("page")) continue;
                buffer.append(kvs[0]);
                if (kvs.length == 2) buffer.append("=").append(kvs[1]);
                buffer.append("&");
            }
            this.url = headUrl + "?" + buffer.toString();
        } else {
            this.url = headUrl + "?";
        }
    }
